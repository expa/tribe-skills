---
name: tribe-web-pages
description: >-
  Build and edit pages on the Tribe AI marketing site (Next.js App Router +
  Payload CMS, Tailwind v4, GSAP/Lenis motion). Covers the page skeleton, the
  shared component library (CTAButton, FadeInSerif/FadeInBody, ScrollCTAButton,
  RiseIn), section rhythm, the CMS-driven-copy rule, and the migrations
  workflow. Use when adding a new page/section to the website or editing an
  existing one. Pairs with tribe-brand for the visual system. Triggers on "new
  page on the site", "add a section", "build a landing page", "edit the
  homepage", "Payload", "tribe website page".
---

# Tribe AI — Web Pages

For building pages in the Tribe website repo. **Read `tribe-brand` first** for
...
<header className="flex flex-col gap-4 max-w-2xl">
  <FadeInSerif as="p" introOnMount inOrder
    className="font-caption text-xs uppercase tracking-[1.6px] text-accent-orange">
    {eyebrow}
  </FadeInSerif>
