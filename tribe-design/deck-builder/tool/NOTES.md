...
- **Local images in standalone decks**: absolute `file:///…` image URLs survive
  the encoder and render in `deck.html` + the headless PDF export (render.mjs
  passes `--allow-file-access-from-files`). Use them for local PDF workflows
  (e.g. art generated by `../article-cover`); the deck then only renders on
  that machine — switch to hosted URLs before sharing a view link.
