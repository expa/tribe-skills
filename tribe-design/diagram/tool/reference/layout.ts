...
// Runs server-side (no DOM), so text sizing is a char-width estimate.

import { type Diagram, type DiagramNode, NODE_MIN_W, NODE_MIN_H, CANVAS_W, CANVAS_H } from './model'
...
function lineCount(text: string, maxChars: number): number {
  let lines = 0
...
function sizeForLabel(label: string, sub?: string): { w: number; h: number } {
  const text = label || ''
...
function rankNodes(nodes: DiagramNode[], edges: Diagram['edges']): Map<string, number> {
...
function orderLayers(layers: string[][], edges: Diagram['edges']): void {
...
// neighbouring nodes (LR arrows run through the between-layer gap horizontally, so
// that gap must clear the label width; TB stacks labels vertically so it needs a
// taller band + wider columns to keep parallel labels from touching).
function place(
  ...
  const mainSize = (id: string) => (orientation === 'LR' ? sized.get(id)!.w : sized.get(id)!.h)
  const crossSize = (id: string) => (orientation === 'LR' ? sized.get(id)!.h : sized.get(id)!.w)
...
export function autoLayout(diagram: Diagram): Diagram {
  ...
  // Nodes with an image keep their current size (already sized to show the
  // image) — only text-only nodes are re-sized to fit their label.
  ...
  // Flip the layering if the flow runs against increasing rank. Longest-path
  // ranking normally puts sources first, but a complex graph with return arrows /
  // no clear source can invert it (the start lands at the bottom). Counting which
  // way edges point makes the dominant flow read top→bottom / left→right; a normal
  // DAG has forward > backward so this is a no-op there.
  let forward = 0
  let backward = 0
  ...
  // Widest edge-label chip (matches the on-canvas chip: chars × 6.5 + 24), so the
  // placement can open enough room between layers for labels to read cleanly.
  ...
  const score = (r: { w: number; h: number }) => Math.abs(Math.log(r.w / r.h / TARGET_ASPECT))
  const chosen = score(lr) <= score(tb) ? lr : tb

  // Center the whole layout within the canvas (so it sits mid-screen), never
