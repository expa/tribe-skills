...
// distributed evenly along it (space-around). Shared by the canvas and the SVG
// export so on-screen and exported diagrams route identically.

import { type Diagram, type DiagramNode } from './model'

export type Pt = { x: number; y: number }
type Side = 'top' | 'right' | 'bottom' | 'left'
export type RoutedEdge = { id: string; label?: string; dashed?: boolean; points: Pt[]; labelPos: Pt }
...
// of kinking. Keep the point this far from the corners; only bother when the
// sides overlap at least this much.
const STRAIGHTEN_MARGIN = 8
const STRAIGHTEN_MIN_OVERLAP = 20

const center = (n: DiagramNode): Pt => ({ x: n.x + n.w / 2, y: n.y + n.h / 2 })

const sideFromDelta = (dx: number, dy: number): Side =>
  Math.abs(dx) >= Math.abs(dy) ? (dx >= 0 ? 'right' : 'left') : dy >= 0 ? 'bottom' : 'top'

// The facing sides for an edge A→B, based on the dominant axis between centres.
function sidesFor(a: DiagramNode, b: DiagramNode): [Side, Side] {
  const ac = center(a)
  const bc = center(b)
...
function badgeOutwardSide(node: DiagramNode, byId: Map<string, DiagramNode>): Side | null {
...
function attachPoint(n: DiagramNode, side: Side, t: number): Pt {
  switch (side) {
    case 'left':
...
// Does an axis-aligned segment (p→q) come within RECT_MARGIN of rect `r`? For
// H/V segments the bounding box IS the segment, so this is exact.
...
// two endpoint boxes and pick the first that clears them (falling back to the
// least-overlapping). With no obstacles the first candidate = the original route.
function orthPoints(p1: Pt, d1: Pt, p2: Pt, d2: Pt, src?: Rect, tgt?: Rect): Pt[] {
  ...
  const mx = (a.x + b.x) / 2
  const my = (a.y + b.y) / 2
  const VH: Pt[] = [{ x: a.x, y: b.y }]
  const HV: Pt[] = [{ x: b.x, y: a.y }]
  const Zv = (x: number): Pt[] => [{ x, y: a.y }, { x, y: b.y }]
  const Zh = (y: number): Pt[] => [{ x: a.x, y }, { x: b.x, y }]
  ...
  // Detours whose linking leg runs beyond BOTH nodes — for same-side pins where
  // the standard shapes all cut across a box.
  order.push(Zv(Math.max(a.x, b.x) + STUB), Zv(Math.min(a.x, b.x) - STUB), Zh(Math.max(a.y, b.y) + STUB), Zh(Math.min(a.y, b.y) - STUB))

  const rects = [src, tgt].filter((r): r is Rect => !!r)
  let best = order[0]
...
// into the first leg). A uniform orientation makes every waypoint a genuine BEND —
// the incoming and outgoing legs meet there on perpendicular axes — so both a
// waypoint's x AND y pin the route (dragging it in any direction reshapes the
// line). Degenerate legs (a waypoint aligned with its neighbour) collapse to a
// straight run via simplify(). Every segment stays horizontal/vertical.
function orthThrough(p1: Pt, d1: Pt, wps: Pt[], p2: Pt, d2: Pt): Pt[] {
  const a = { x: p1.x + d1.x * STUB, y: p1.y + d1.y * STUB }
  const b = { x: p2.x + d2.x * STUB, y: p2.y + d2.y * STUB }
  const chain = [a, ...wps, b]
  ...
  for (let i = 1; i < chain.length; i++) {
    const u = chain[i - 1]
    const v = chain[i]
    // The elbow sits so the leg LEAVING u uses the seeded axis and the leg
    // ARRIVING at v uses the other — putting the bend on v (the waypoint).
...
// t = 0.5 → the midpoint (the default label position).
...
// (0 → sharp 90° corners). Shared by the canvas and the export.
export function edgePath(points: Pt[], radius = 0): string {
...
export function routeEdges(diagram: Diagram): RoutedEdge[] {
  const byId = new Map(diagram.nodes.map((n) => [n.id, n]))
  type EP = { edgeId: string; end: 'from' | 'to'; side: Side; other: DiagramNode }
  const groups = new Map<string, EP[]>()
  const sides = new Map<string, { fromSide: Side; toSide: Side; from: DiagramNode; to: DiagramNode }>()

  const push = (key: string, ep: EP) => {
  ...
  for (const e of diagram.edges) {
    ...
    // A manually-routed edge leaves each node toward its nearest waypoint, so the
    // exit side follows the first waypoint (source) / last waypoint (target).
  ...
  const tByEndpoint = new Map<string, number>()
  ...
  for (const e of diagram.edges) {
    ...
    // endpoint toward the other one so a right→left (or bottom→top) arrow runs
    // straight even when the nodes differ in height or y (avoiding the S-kink).
    // This works with several arrows sharing a side too: each slides from its own
    // distributed position, and the slide cap (25% of the side) is divided by the
    // number of arrows on that side so they don't pile onto each other.
